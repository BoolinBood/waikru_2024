...
const AdminPage = () => {
  const { isConnected, dbConnected, transport, trays, deleteTray } = useAppContext();
      ...
      <ul className='space-y-3'>
          {trays.map((tray) => (
            <li key={tray._id} className='flex gap-3 items-center'>
              <span>{tray.name} - {tray.message} - {tray.selectedTray}</span>
              <button className='text-white px-2 py-1 text-sm bg-red-500 rounded-lg hover:bg-red-500/90' onClick={() => deleteTray(tray._id)}>Delete</button>
            </li>
          ))}
        </ul>
    </div>
